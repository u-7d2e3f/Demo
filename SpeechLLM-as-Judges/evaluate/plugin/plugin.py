...
class MyMultiTaskReward(ORM):
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        for content, sol in zip(completions, solution):
            ...
            reward = 0.0
            try:
                # 从 solution 提取 <task> 与后续文本（作为参考 reference）
                m_task = re.search(r"<task>(.*?)</task>", sol, flags=re.S | re.I)
                m_tail = re.search(r"</task>(.*)", sol, flags=re.S | re.I)
                task = (m_task.group(1).strip() if m_task else "SingleEval")
                reference = (m_tail.group(1).strip() if m_tail else sol.strip())

                if task == "SingleEval":
                    # 用 DeepSeek 打分（0~10），作为 reward
                    reward = self.score_deepseek(task=task,
                                                 reference=reference,
                    ...
                    reward = self.score_deepseek(task=task,
                                 reference=reference,
                                 candidate=str(content))
                elif task == "CompareEval":
                    reward = self.score_deepseek(task=task,
                                 reference=reference,
                                 candidate=str(content))
                elif task == "FakeDetection":
                ...
                if self.count % 100 == 0:
                    print(f"[MyMultiTaskReward] task: {task}, reward: {reward}, content: {content}, solution: {sol}")

            except Exception:
                print(f"[MyMultiTaskReward] Error processing completion: {content}, solution: {sol}")
                reward = 0.0
            rewards.append(float(reward))
        return rewards

    # ============== DeepSeek 调用与通用打分（0~10） ==============

    def score_deepseek(self, task: str, reference: str, candidate: str,
                   max_retries: int = 3, sleep_s: float = 0.8) -> float:
        build = {
            "SingleEval": self._prompt_single_eval,
            "Suggest": self._prompt_suggest,
            "CompareEval": self._prompt_compare_eval,
        }.get(task, self._prompt_single_eval)

        messages = build(reference, candidate)
        ...
        for attempt in range(max_retries):
            try:
                resp = self.client.chat.completions.create(
                    ...
                    temperature=0.0,
                    top_p=1.0,
                    max_tokens=16,   # 输出只有一个整数，进一步压小
                    stream=False
                )
                text = resp.choices[0].message.content or ""
                score_int = self._extract_score(text)   # ← 现在返回 0~10 的整数
                if score_int is not None:
                    # 归一化到 [0,1]，保留一位小数
                    return max(0.0, min(1.0, round(score_int / 10.0, 1)))
            except Exception as e:
                print(f"[MyMultiTaskReward] DeepSeek attempt {attempt+1}/{max_retries} failed: {e}")

            if attempt < max_retries - 1:
                time.sleep(sleep_s)
        return 0.0

    # ---------------- Prompt 构造（SingleEval） ----------------
    def _prompt_single_eval(self, reference: str, candidate: str):
        """
        只返回 0~10 的一位整数；不输出任何解释或前后缀。
        reference: 标注/参考答案（</task> 后文本）
        candidate: 模型输出（待评估）
        """
        user_msg = f"""You are an evaluator assessing the quality of an AI-generated response based on a provided human-labeled context and a human-written ground-truth response.
...
[Instructions]
Evaluate Candidate relative to Reference on the criteria above and determine an overall quality score directly on a 0–10 scale.

[Output Rules — CRITICAL]
- Output ONLY one integer on a single line.
- Range: 0 to 10 (inclusive).
- No words, labels, punctuation, code fences, or extra text.

[Final Output]
<an integer between 0 and 10>
"""
        return [{"role": "user", "content": user_msg}]
    def _prompt_suggest(self, reference: str, candidate: str):
        """
        对“改进建议”类回答做评估：只返回 0~10 ；不输出任何解释或前后缀。
        reference: 人工撰写/标准的改进建议（</task> 后文本）
        candidate: 模型给出的改进建议
        """
        user_msg = f"""You are an evaluator assessing the quality of an AI-generated response based on a provided human-labeled context and a human-written ground-truth response.
...
[Instructions]
Evaluate Candidate Suggestions relative to the Reference on the criteria above and determine an overall quality score directly on a 0–10 scale.
...
"""
    ...
    def _prompt_compare_eval(self, reference: str, candidate: str):
        """
        比较类评测（CompareEval）：只返回 0~10 ；不输出任何解释或前后缀。
        reference: 人工标注的“正确偏好/比较结论+依据”（</task> 后文本）
        candidate: 模型给出的比较/判别与理由（待评估）
        """
        user_msg = f"""You are an evaluator assessing the quality of an AI-generated response based on a provided human-labeled context and a human-written ground-truth response.

[Reference Comparison Evaluation]
{reference}

[Candidate Comparison Evaluation]
{candidate}
...
[Instructions]
Evaluate the Candidate relative to the Reference on the criteria above and determine an overall quality score directly on a 0–10 scale.
...
"""
    ...
    def _extract_score(text: str) -> Optional[int]:
        """
        从模型输出中抽取 0~10 的整数；解析失败返回 None。
        """
        if not text:
            return None

        # 优先匹配独立的 0~10 整数（避免误抓 100/101 等）
        m = re.search(r"\b(10|[0-9])\b", str(text).strip())
        if m:
            try:
                val = int(m.group(1))
                if 0 <= val <= 10:
...
class MyMultiTaskQwenReward(ORM):
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        for content, sol in zip(completions, solution):
            ...
            try:
                ...
                # 提取<answer>标签中的(同时兼容不带标签的)
                content_re = re.search(r"<answer>(.*?)</answer>", content, flags=re.S | re.I)
                content = content_re.group(1).strip() if content_re else content

                reference_re = re.search(r"<answer>(.*?)</answer>", reference, flags=re.S | re.I)
                reference = reference_re.group(1).strip() if reference_re else reference
...
class MyMultiTaskQwenHelpfulnessReward(ORM):
    def __init__(self) -> None:
        self.client =  OpenAI(
    api_key='EMPTY',
    base_url=f'http://localhost:8000/v1',
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        for content, sol in zip(completions, solution):
            ...
            try:
                ...
                # 提取<answer>标签中的(同时兼容不带标签的)
                content = re.search(r"<answer>(.*?)</answer>", content, flags=re.S | re.I)
                content = content.group(1).strip() if content else content
                reference_re = re.search(r"<answer>(.*?)</answer>", reference, flags=re.S | re.I)
                reference = reference_re.group(1).strip() if reference_re else reference
...
[Evaluation Focus]
    - Helpfulness: Does the answer provide useful and relevant information?
    8–10 — Directly usable and targeted: gives specific actions (e.g., reduce minor distortion at 0–0.5s, 1.3–2.1s, 3.1–3.8s via denoise/retake; adjust pacing; widen pitch/energy for expressiveness).
    6–8 — Actionable plan with minor gaps; mostly tailored; at least one check or fallback is suggested.
    4–6 — Partly useful: identifies issues (distortion, limited emotional engagement) but lacks precise locations/parameters/practice details.
    2–4 — Mostly generic advice (e.g., “improve clarity/intonation”) with little operational detail; not readily applicable.
    0–2 — Barely helpful or misleading; advice doesn’t fit the context or could make results worse.
...
[Evaluation Focus]
    - Helpfulness: Does the answer provide useful and relevant information?
    8–10 — Directly improves the piece: prioritized, actionable steps with clear how-to.
    6–8 — Solid and usable, but misses one element (priority, verification, or justification).
    4–6 — Partly helpful: directions are generic or incomplete; user must figure out key details to execute.
    2–4 — Mostly platitudes (“be more expressive”); little practical value without extra work.
    0–2 — Misleading or counterproductive suggestions; could worsen quality.
...
[Evaluation Focus]
    - Helpfulness: Does the answer provide useful and relevant information?
    8–10 — Clear decision (A/B or justified tie) with ranked reasons that map to the brief’s goals; highlights where and why one sample is better (e.g., “A cleaner in 0–3.2 s; more balanced dynamics in verse lines”), plus what that implies (intelligibility, listening comfort). If close, states trade-offs and when to prefer each.
    6–8 — Decision is usable with specific reasons, but lacks either prioritization, trade-offs, or implications.
    4–6 — Gives a decision but reasons are partly generic or mixed; reader must infer the main takeaway.
    2–4 — Vague preference (“A sounds nicer”) with minimal justification; weak for decision-making.
    0–2 — No decision, or misleading guidance that would pick the worse sample.
...
class MyMultiTaskQwenRelevanceReward(ORM):
    def __init__(self) -> None:
        self.client =  OpenAI(
    api_key='EMPTY',
    base_url=f'http://localhost:8000/v1',
...
[Evaluation Focus]
    - Relevance: Does it stay on topic and align with the context?
    8–10 — Stays tightly on clarity/intelligibility, pacing, distortion with timestamps, dynamic range stability, tone/timbre, expressiveness/emotion; no irrelevant digressions; follows the reference.
    6–8 — Generally focused with brief, still-useful tangents; constraints are respected with only trivial deviations.
    4–6 — Mostly relevant but includes nontrivial extraneous content or overlooks a minor stated point (e.g., underemphasizes dynamic range being stable).
    2–4 — Weak focus: substantial digressions or misses a key element from the context (e.g., ignores the provided distortion intervals).
    0–2 — Largely off-topic or at odds with instructions/context.
...
[Evaluation Focus]
    - Relevance: Does it stay on topic and align with the context?
    8–10 — Tightly aligned to the observed issues (e.g., rate, emotional nuance, expressiveness) and any provided constraints; no off-topic or format violations (stays in “suggest” mode, not scoring/explaining).
    6–8 — Mostly on-topic, with brief tangents that still help.
    4–6 — Some relevance, but includes nontrivial digressions or overlooks a stated focus.
    2–4 — Weak alignment; misses key issues mentioned/implied in context.
    0–2 — Largely off-topic or contradicts instructions.
...
[Evaluation Focus]
    - Relevance: Does it stay on topic and align with the context?
    8–10 — Tightly focused on A vs B across the specified axes (clarity/intelligibility, pacing, distortion with timestamps, dynamic range, tone/expressiveness); no unrelated commentary; follows output constraints (e.g., compare rather than re-summarize background).
    6–8 — Mostly on-topic with brief tangents that still inform the comparison.
    4–6 — Some relevant content, but includes nontrivial digressions or underplays a required axis.
    2–4 — Weak alignment: talks about single-sample qualities without comparing, or ignores key axes.
    0–2 — Off-topic or contradicts the comparison instruction (e.g., evaluates only A).
...
class MyMultiTaskQwenAccuracyReward(ORM):
    def __init__(self) -> None:
        self.client =  OpenAI(
    api_key='EMPTY',
    base_url=f'http://localhost:8000/v1',
...
[Evaluation Focus]
    - Accuracy: Does it reflect the facts stated in the context?
    8–10 — Fully consistent with the reference: correctly notes every details; no fabricated metrics or contradictions.
    6–8 — Minor imprecision that doesn’t change the outcome (e.g., “slight” vs. “minor”); overall alignment remains correct.
    4–6 — Some incorrect details or unsupported claims, but the main idea is salvageable.
    2–4 — Key error or contradiction; parts may remain usable.
    0–2 — Predominantly incorrect or fabricated; contradicts core facts; unsafe/misleading.
...
[Evaluation Focus]
    - Accuracy: Does it reflect the facts stated in the context?
    8–10 — Suggestions match the evidence (no invented problems), choose appropriate, safe remedies, and avoid technical/myth errors.
    6–8 — Minor imprecision but generally appropriate; outcomes remain valid.
    4–6 — Some mismatches yet partially useful.
    2–4 — Major mismatch or questionable technique likely to underperform.
    0–2 — Fabricated issues, unsafe/incorrect advice, or contradictions with context.
...
[Evaluation Focus]
    - Accuracy: Does it reflect the facts stated in the context?
    8–10 — Consistent with context: correctly attributes issues. No fabricated timestamps/metrics; no internal contradictions; terminology used correctly.
    6–8 — Minor imprecision that doesn’t change the outcome; overall alignment intact.
    4–6 — Some incorrect or unsubstantiated claims, but the main verdict is still plausible.
    2–4 — Major misread of evidence leading to doubtful verdict.
    0–2 — Largely inaccurate/fabricated; contradicts core facts or uses faulty audio concepts.
...
class MyMultiTaskQwenDetailReward(ORM):
    def __init__(self) -> None:
        self.client =  OpenAI(
    api_key='EMPTY',
    base_url=f'http://localhost:8000/v1',
...
[Evaluation Focus]
    - Level of Detail: Is the response thorough and precise?
    8–10 — Reproducible detail: cites exact timestamps for issues; separates objective vs. subjective aspects.
    6–8 — Strong detail with minor omissions (eg. missing one timestamp or a small validation step); still largely reproducible.
    4–6 — Main points present but lacks important parameters; some ambiguity remains.
    2–4 — High-level outline; major gaps.
    0–2 — Little to no actionable detail; mostly restates the prompt or uses vague descriptors.
...
[Evaluation Focus]
    - Level of Detail: Is the response thorough and precise?
    8–10 — Specific targets, concrete how-to steps and actionables.
    6–8 — Detailed but missing one element.
    4–6 — Main areas named, but lacks crucial parameters/examples; ambiguity remains.
    2–4 — High-level bullets only; little to reproduce in practice.
    0–2 — Vague or filler; no operational detail.
...
[Evaluation Focus]
    - Level of Detail: Is the response thorough and precise?
    8–10 — Concrete evidence: cites timestamps/segments for differences; separates objective from subjective.
    6–8 — Specific and useful but missing one element.
    4–6 — Names main areas but have some ambiguity.
    2–4 — High-level bullets with little localization or evidence; hard to verify.
    0–2 — Vague restatements; no actionable or verifiable detail.
...
orms['external_my_multitask'] = MyMultiTaskReward
orms['external_my_multitask_qwen'] = MyMultiTaskQwenReward

orms['external_my_multitask_qwen_help'] = MyMultiTaskQwenHelpfulnessReward
orms['external_my_multitask_qwen_relevance'] = MyMultiTaskQwenRelevanceReward
orms['external_my_multitask_qwen_accuracy'] = MyMultiTaskQwenAccuracyReward
orms['external_my_multitask_qwen_detail'] = MyMultiTaskQwenDetailReward
